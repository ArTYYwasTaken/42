...
// Prototype for the custom ft_memmove function
void *ft_memmove(void *dest, const void *src, size_t n);

// Function to print memory content for debugging
...
// Test function to compare ft_memmove and memmove
void test_memmove(const char *test_name, unsigned char *dest1, unsigned char *dest2, const unsigned char *src, size_t n) {
    printf("Test: %s\n", test_name);
...
int main() {
    ...
    test_memmove("Overlapping regions (dest before src)", src2 + 7, src2 + 7, src2, 6);

    // Test case 3: Overlapping regions (src before dest)
    unsigned char src3[] = "Hello, World!";
    test_memmove("Overlapping regions (src before dest)", src3, src3, src3 + 7, 6);

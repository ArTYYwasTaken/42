...
char	*ft_strdup(const char *s)
{
	...
	arrlen = ft_strlen(s) + 1; // Calculate the length of the string plus one for the null terminator
	arr = malloc(arrlen * sizeof(char)); // Allocate memory for the new string
	if (!arr) // Check if memory allocation was successful
		return (NULL);
	ft_strlcpy(arr, s, arrlen); // Copy the string into the newly allocated memory
